...
#define B_HIDDEN_POWER_DMG          GEN_5      // In Gen6+, Hidden Power's base power was set to always be 60. Before, it was determined by the mon's IVs.
...
#define B_UPROAR_TURNS              GEN_4      // In Gen5+, Uproar lasts for 3 turns instead of 2-5 turns.
...
#define B_UPDATED_CONVERSION        GEN_5      // In Gen6+, Conversion changes the user's type to match their first move's. Before, it would choose a move at random.
...
#define B_MINIMIZE_EVASION          GEN_4      // In Gen5+, Minimize raises evasion by 2 stages instead of 1.
...
#define B_CRASH_IF_TARGET_IMMUNE    GEN_3      // In Gen4+, The user of Jump Kick or High Jump Kick will "keep going and crash" if it attacks a target that is immune to the move.
...
#define B_NEST_BALL_MODIFIER        GEN_5      // Nest Ball's formula varies depending on the Gen. See Cmd_handleballthrow.
...
#define VAR_TERRAIN                 VAR_KEEP_TERRAIN  // If this var has a value, assigning a STATUS_FIELD_xx_TERRAIN to it before battle causes the battle to start with that terrain active
